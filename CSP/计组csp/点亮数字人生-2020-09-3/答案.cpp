...
int main()
{
    ...
    for(int i=0;i<qt_num;i++)
    {
        ...
        while(1)
        {
                ...
                if(!dce[i].ct)//未被建造
                {
                    ...
                    for(int j=0;j<dce[i].input_s.size();j++)
                    {
                        ...
                        if(!dce[i].id_flag[j])//未访问I/O
                        {
                            ...
                            if(run_level > dce[i].level)//level大进行层级更新
                            {
                    ...
                    {
                        //查找有无未访问I/O
                        if(!dce[i].id_flag[j]) break;
                        //标识该器件已被建造
                        if(j==dce[i].id_flag.size()-1) dce[i].ct = true;
                    }
                }
            }
            //判断电路是否成环
            ...
            for(int i=0;i<d_num;i++)
            {
                if(pre_level[i]==dce[i].level) same_num++;//每执行一次各器件层级应该会发生变化，
            }                                             //没有变化说明电路存在环
            if(same_num==d_num)
            {
                loop = true;
        ...
        else
        {
            ...
            for(int i=0;i<test_num;i++)
            {
                ...
                for(int j=0;j<d_num;j++)
                {
                    ...
                    {
                        if(dce[j].input_s[k][0]=='I')//去test_input里面找对应的输入
                            dce[j].input.push_back(test_input[i][str_to_int(dce[j].input_s[k])-1]);
                        else if(dce[j].input_s[k][0]=='O')//去output里面找对应的输出
